...
export function buildProjection(state: AppState, today: DateKey): Projection {
  return {
    steps: panelSteps(state.logs, state.habits, today),
    preview: panelSteps(state.logs, state.habits, today, { includeCurrentPeriod: true }),
    fullDay: isFullDay(indexLogs(state.logs).get(today), state.habits, today),
  };
}
